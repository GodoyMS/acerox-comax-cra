    ...
    <>
      <div style={{ zIndex: 200 }}>
        <button
          ...
          }  p-2   rounded-lg flex justify-center items-center gap-4 `}
        >
          <FaCartPlus className=" w-6 h-6" />
          <span className=" font-bold text-sm hidden lg:block">
            Carrito ( {productsRedux && productsRedux.length} )
          </span>
        </button>
        {isCarritoComprasActive && (
          <div
            style={{ maxHeight: "500px" }}
            className="absolute top-full right-0  max-w-xs    overflow-y-scroll   justify-center rounded-lg  "
          >
            <div className="w-full flex-col gap-2 flex bg-gray-200 dark:bg-gray-700 rounded-md   ">
              ...
              {productsRedux.length > 0 && (
                <div className="p-4 flex justify-center">
                  <Link
                    onClick={() => setIsCarritoComprasActive(false)}
                    to={"/cotizar"}
                    className="text-white bg-tertiary hover:bg-tertiaryhover flex justify-center transition duration-300 px-4 py-2 font-bold w-full rounded-md"
                  >
                    Cotizar
                  </Link>
                </div>
